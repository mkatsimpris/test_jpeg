...
def triple_block_buffer(inputs, outputs, clock, reset, line_width=16):
    ...
    buffer_select_write = Signal(intbv(0, min=0, max=3))
    buffer_select_read = Signal(intbv(0, min=0, max=3))
    ...
    @always_seq(clock.posedge, reset=reset)
    def output_assign():
        if block_buffer_out_1.data_valid:
            outputs.data_out.next = block_buffer_out_1.data_out
        elif block_buffer_out_2.data_valid:
            outputs.data_out.next = block_buffer_out_2.data_out
        elif block_buffer_out_3.data_valid:
    ...
    return (block_buffer_1, block_buffer_2, block_buffer_3, data_valid_assign, output_assign,
            read_mux, write_mux, read_counter_update, write_counter_update, data_input,
            or_signals)
